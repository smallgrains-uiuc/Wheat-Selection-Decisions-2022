...
library(gaston)
library(rrBLUP)
all<- read.csv('ILTrainingSetPhenotypesJuly.20.2022.csv', row.names=1)
...
#Remove Urbana 2020 data because of its negative correlation with other sites
allsub<- droplevels.data.frame(all[which(all$year>19),])
...
if(twoME){
  ...
  south_locs<- unique(allsub$loc)[-grep('Urb|Scb|War|Blk', unique(allsub$loc))]
  north_locs<- c('Urb')
  ...
  traits_sub<- c('North.Grain.yield...bu.ac', 'South.Grain.yield...bu.ac', 
                 'Grain.test.weight...lbs.bu',
